...
1. **Dennis Snellenberg (dennissnellenberg.com)**:
   ...
   - **Implementation**: GSAP (GreenSock), Lenis/Locomotive Scroll for scroll smoothing, and custom SVG path animations.
...
- **Custom Shaders (GLSL)**:
  - For image hover effects, use a custom fragment shader to create wave distortions or liquid morphs using noise textures.
  - For interactive 3D elements, use a custom vertex shader to morph abstract geometric shapes responsive to cursor coordinates or scroll.
- **Performance**:
...
- **Custom Cursors**:
  - A custom follower cursor that scales up, morphs, or shows text depending on what the user hovers over.
- **Magnetic Buttons**:
  - Elements that pull the cursor towards them slightly when the cursor is near.
- **Smooth Scroll**:
  - Use **Lenis** to normalize scrolling behavior and ensure fluid WebGL synchronization.
...
- `src/shaders/`: Houses vertex and fragment shaders (`.vert`, `.frag`).
